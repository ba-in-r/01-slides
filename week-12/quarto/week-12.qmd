...
| **Confianza mínima (`conf = 0.5`)** | 50 % | Solo se conservan reglas que predicen el consecuente con al menos un 50 % de probabilidad. |
...
## Convertir a data frame para inspeccionar
reglas.df <- as(reglas, "data.frame")
reglas.df
```

Ahora reglas contiene solo las reglas no redundantes, lo que mejora la claridad del análisis y evita repeticiones. El nuevo reglas.df muestra el conjunto final depurado, con las métricas principales de soporte, confianza y lift.
...
plot(reglas.df, measure = c("support", "confidence"), shading = "lift")
```
