---
title: "Unidad 7"
subtitle: "Semana 11: Visualización y Análisis de Redes (Teoría de Grafos)"
...
Matemáticamente:
$$A_{ij} = A_{ji}$$
donde $A_{ij}$ indica la existencia de un vínculo entre los nodos *i* y *j*.
...
1. **Lista de aristas (edge list):**
   Cada fila indica una conexión entre dos nodos:
...
2. **Matriz de adyacencia:**
   Matriz cuadrada $A$ donde $A_{ij} = 1$ si *i* está conectado con *j*, y 0 si no.

3. **Matriz de incidencia:**
   Representa qué aristas inciden en qué nodos, útil para cálculos estructurales.
...
- **Grado de centralidad (Degree):**
  Número de conexiones directas que tiene un nodo.
  $$C_D(i) = \sum_j A_{ij}$$
  Cuanto mayor el grado, mayor la capacidad de interacción directa.

- **Centralidad de intermediación (Betweenness):**
  Mide cuántas veces un nodo actúa como **puente** en los caminos más cortos entre otros nodos.
  $$C_B(i) = \sum_{s \neq i \neq t} \frac{\sigma_{st}(i)}{\sigma_{st}}$$
  donde $\sigma_{st}(i)$ es el número de caminos mínimos entre *s* y *t* que pasan por *i*.

### Métricas globales
Evalúan la posición de un nodo **respecto a toda la red**.

- **Centralidad de cercanía (Closeness):**
  Indica qué tan rápido puede un nodo llegar a todos los demás.
...
- La **modularidad (Q)** mide qué tan bien separadas están esas comunidades:
...
A partir de esta red podemos responder preguntas como:
	...
	•	¿Qué tan conectada es la red del equipo?

Este ejercicio nos permitirá visualizar cómo los patrones de interacción dentro del equipo pueden analizarse con las mismas herramientas que usamos para estudiar redes sociales, empresariales o financieras. En esencia, un equipo de fútbol es también un sistema de relaciones, donde la posición en la red revela liderazgo, dependencia y eficiencia colectiva.
...
library(igraph)      # Construcción y análisis de redes
library(ggraph)      # Visualización avanzada de grafos
...
## plot
plot(pond_dir , edge.width=E(pond_dir)$weight ,  edge.arrow.size = 0.8)
```
